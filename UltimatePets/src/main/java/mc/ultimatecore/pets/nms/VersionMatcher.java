...
public class VersionMatcher {
    ...
    public NMS match() {
        ...
        try {
            return (NMS) Class.forName("mc.ultimatecore.pets.nms.v" + rVersion)
                    .getDeclaredConstructor()
                    .newInstance();
        } catch (ClassNotFoundException exception) {
            HyperPets.getInstance().getLogger().warning("Unsupported Version Detected: " + rVersion);
            Bukkit.getPluginManager().disablePlugin(HyperPets.getInstance());
